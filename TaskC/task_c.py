...
def convert_reservation_data(reservation: list) -> list:
    ...
    converted.append(
        datetime.strptime(reservation[10].strip(), "%Y-%m-%d %H:%M:%S")
    )  # createdAt (datetime)
    return converted
...
def confirmed_reservations(reservations: list[list]) -> None:
    ...
    for reservation in reservations:
        if reservation[8]:
            print(
                f"- {reservation[1]}, {reservation[9]}, {reservation[4].strftime("%d.%m.%Y")} at {reservation[5].strftime("%H.%M")}"
            )
...
def long_reservations(reservations: list[list]) -> None:
    ...
    for reservation in reservations:
        if reservation[6] >= 3:
            print(
                f"- {reservation[1]}, {reservation[4].strftime("%d.%m.%Y")} at {reservation[5].strftime("%H.%M")}, duration {reservation[6]} h, {reservation[9]}"
            )
